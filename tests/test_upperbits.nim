...
type
  Owner = ref object
    name: string
    subject: UpperBitsTagged[Subject]
  Subject = ref object
    name: string
    owner: UpperBitsTagged[Owner]
...
block: # simple cycle
  ...
  var subjectA = Subject(name: "A", owner: tagUpperBits(owner, 5))
  owner.subject = tagUpperBits(subjectA, 7)
